...
def clean_up_sentence(sentence):
  # tokenize pattern - splitting words into array
...
def predict_class(sentence):
  # filer below thershold predictions
...
def getResponse(ints, intents_json):
  tag = ints[0]['intent']
  lis_of_intents = intents_json['intents']
...
def seend():
  ...
  if msg != '':
    ...
    ChatBox.insert(END, f"Bot: {res} \n\n")
    
    ChatBox.config(state=DISABLED)
    ChatBox.yview(END)
    
root = Tk()
root.title("IRIS Assistant")
